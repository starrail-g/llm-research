...
- 均方误差：
  \[ J(\boldsymbol{\theta}) = \frac{1}{2m} \lVert X\boldsymbol{\theta} - \mathbf{y} \rVert^2
  = \frac{1}{2m}(X\boldsymbol{\theta}-\mathbf{y})^\top (X\boldsymbol{\theta}-\mathbf{y}). \]
- **展开成三项**：
...
- **具体到线性回归**：
  \[ \nabla J(\boldsymbol{\theta})
    = \frac{1}{2m} \nabla\bigl(\boldsymbol{\theta}^\top X^\top X\boldsymbol{\theta} - 2\boldsymbol{\theta}^\top X^\top\mathbf{y} + \mathbf{y}^\top\mathbf{y}\bigr)
    = \frac{1}{m}\bigl(X^\top X\boldsymbol{\theta} - X^\top\mathbf{y}\bigr).
  \]
- **批量梯度表达**：
...
- **目标与梯度**：
  \[ J = \frac{1}{2m}\lVert X\boldsymbol{\theta}-\mathbf{y}\rVert^2,
     \quad \nabla J = \frac{1}{m}X^\top(X\boldsymbol{\theta}-\mathbf{y}). \]
- **正则化扩展**：可在损失中加入 \(L1\) 或 \(L2\) 正则项，控制模型复杂度。

---

> **小结**：从一元到多元，线性回归核心在于“构造矩阵”、“定义损失”、“求梯度”与“更新参数”，扩展只需增减特征维度或添加正则化。